...
public class GestorVentas implements IGestorVentas{
    ...
    @Override
    public boolean procesarVenta(List<dtos.DetalleVentaDTO> carrito, dtos.ClienteDTO cliente, double efectivoRecibido) {
        ...
        if (efectivoRecibido >= total) {
            ...
            VentaDTO venta = new VentaDTO(folioVenta, carrito, cliente, total, "Efectivo");
    ...
    public double calcularKilosTotales(List<VentaDTO> ventas) {
        return ventas.stream().filter(v -> v.getProductos() != null) .flatMap(v -> v.getProductos().stream()) .mapToDouble(dtos.DetalleVentaDTO::getCantidadKilos).sum(); 
    }
