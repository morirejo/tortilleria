...
public class ControlPresentacionVenta {
    ...
    public void solicitarCobro(double efectivoRecibido, javax.swing.JFrame pantallaActual) {
        if (efectivoRecibido >= totalActual) {
            java.util.List<DetalleVentaDTO> carrito = new java.util.ArrayList<>();
            carrito.add(new DetalleVentaDTO("Tortilla", kilosActuales, totalActual));
            dtos.ClienteDTO cliente = null; 
            boolean exito = controlNegocio.confirmarVenta(carrito, cliente, efectivoRecibido);
            if (exito) {
                pantallaActual.dispose();
                new PantallaExito(this, kilosActuales, totalActual).setVisible(true);
            } else {
                javax.swing.JOptionPane.showMessageDialog(pantallaActual, "Error al guardar en la base de datos.");
            }
        } else {
            javax.swing.JOptionPane.showMessageDialog(pantallaActual, "El pago ingresado es insuficiente.");
        }
    }
